...
get_linked_repos() {
  ...
  # Rename a brain in the configuration
  rename_brain_config() {
      local old_name="$1"
      local new_name="$2"
      local new_path="$3"
      ...
      # 1. Copy the object to new key with new path
      # 2. Delete old key
      # 3. Update 'current' if needed
      jq --arg old "$old_name" --arg new "$new_name" --arg path "$new_path" '
          .brains[$new] = .brains[$old] | 
          .brains[$new].path = $path |
          del(.brains[$old]) |
          if .current == $old then .current = $new else . end
      ' "$CONFIG_FILE" > "$tmp_file" && mv "$tmp_file" "$CONFIG_FILE"
  }
