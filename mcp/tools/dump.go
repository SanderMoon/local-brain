...
func RegisterDumpTools(srv *mcp.Server, sess *session.Session) error {
	// add_task_to_dump
	type AddTaskToDumpArgs struct {
		Content string `json:"content" jsonschema:"Task content"`
	}
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "add_task_to_dump",
		Description: "Quick capture a task to the inbox",
	}, func(ctx context.Context, req *mcp.CallToolRequest, args AddTaskToDumpArgs) (*mcp.CallToolResult, any, error) {
		// Validate inputs
		...
		return &mcp.CallToolResult{
			Content: []mcp.Content{
				&mcp.TextContent{Text: fmt.Sprintf("Added task to inbox: %s", args.Content)},
			},
		}, nil, nil
	})

	// add_note_to_dump
	type AddNoteToDumpArgs struct {
		Title   string `json:"title" jsonschema:"Note title"`
		Content string `json:"content" jsonschema:"Note content (can be multi-line)"`
	}
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "add_note_to_dump",
		Description: "Quick capture a note to the inbox",
	}, func(ctx context.Context, req *mcp.CallToolRequest, args AddNoteToDumpArgs) (*mcp.CallToolResult, any, error) {
		// Validate inputs
		...
		return &mcp.CallToolResult{
			Content: []mcp.Content{
				&mcp.TextContent{Text: fmt.Sprintf("Added note to inbox: %s", args.Title)},
			},
		}, nil, nil
	})
